fn encode_code_points_to_str(chars: &[char]) -> Result<String, std::string::FromUtf8Error> {
...
fn encode_code_points(chars: &[char]) -> Vec<u32> {
    chars.iter().copied().map(encode_one).collect()
}

fn encode_one(c: char) -> u32 {
    const LUT: [u32; 22] = [
...
fn print_encode_str(s: &str) {
    let chars: Vec<_> = s.chars().collect();
    let encoded = encode_code_points(&chars);
    println!("{encoded:?}");
    let encoded = encode_code_points_to_str(&chars);
    println!(" => {encoded:?}");
}
